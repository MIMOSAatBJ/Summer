...
public class AES {
	...
	public static byte[] encrypt(String content, String secretkey) {
		try {
			...
			//使用SecretKeySpec类来根据一个字节数组构造一个 SecretKey,，而无须通过一个（基于 provider的）SecretKeyFactory.
			SecretKeySpec key = new SecretKeySpec(enCodeFormat,GENERATOR);
			//创建密码器 //为创建 Cipher对象，应用程序调用 Cipher 的 getInstance 方法并将所请求转换 的名称传递给它。还可以指定提供者的名称（可选）。
...
//		}
	...
	public static byte[] decrypt(byte[] content, String secretkey) {
		try {
			...
			byte[] enCodeFormat = secretKey.getEncoded();
			SecretKeySpec key = new SecretKeySpec(enCodeFormat,GENERATOR);
			Cipher cipher = Cipher.getInstance(GENERATOR);// 创建密码器
